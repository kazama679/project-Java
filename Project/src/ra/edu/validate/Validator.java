...
public class Validator {
    ...
    public static LocalDate validDate(Scanner sc, LocalDate min, LocalDate max) {
        while(true){
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            try {
                LocalDate inputDate = LocalDate.parse(sc.nextLine(), formatter);
                if (inputDate.isBefore(min)) {
                    PrintError.println("Ngày nhập vào phải lớn hơn hoặc bằng " + min.format(formatter) + ". Vui lòng nhập lại!");
                    continue;
                }
                if (inputDate.isAfter(max)) {
                    PrintError.println("Ngày nhập vào phải nhỏ hơn hoặc bằng " + max.format(formatter) + ". Vui lòng nhập lại!");
                    continue;
                }
                return inputDate;
    ...
    public static int ValidYearSearch(Scanner sc, int min, int max) {
        while(true){
            try {
                int n = Integer.parseInt(sc.nextLine());
                if(n<min){
                    PrintError.println("Phải lớn hơn hoặc bằng năm "+min);
                    continue;
                }
                if(n>max){
                    PrintError.println("Phải nhỏ hơn hoặc bằng năm "+max);
                    continue;
                }
                return n;
            } catch (NumberFormatException e1) {
                ...
                PrintError.println("Có lỗi không xác định khi validate");
            }
        }
    }
}
