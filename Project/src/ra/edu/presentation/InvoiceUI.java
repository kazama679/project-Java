...
public class InvoiceUI {
    ...
    public static void displayInvoiceBySelection(List<Invoice> invoices){
        System.out.println("+----+--------------------------------+------------+------------------+-------------------+");
    ...
    public static void displayInvoice(Scanner scanner){
        ...
        while(true){
            ...
            displayInvoiceBySelection(invoices);
            if (offset == 0) {
    ...
    public static void searchDate(Scanner scanner){
        ...
        LocalDate date = Validator.validDate(scanner, LocalDate.MIN, LocalDate.now());
        List<Invoice> invoices = invoiceService.findByDate(date);
        if(invoices==null || invoices.isEmpty()){
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            PrintError.println("Không tìm thấy hóa đơn của ngày: "+Color.BLUE+date.format(formatter)+Color.RESET+Color.RED+"!"+Color.RESET);
            return;
        }
        displayInvoiceBySelection(invoices);
    }
    ...
    public static void totalRevenueDay(Scanner scanner){
        ...
        LocalDate dateStart = Validator.validDate(scanner, LocalDate.MIN, LocalDate.now());
        System.out.println("Nhập vào ngày kết thúc: ");
        LocalDate dateEnd = Validator.validDate(scanner, dateStart, LocalDate.now());
    ...
    public static void totalRevenueYear(Scanner scanner){
        System.out.println("Nhập vào năm bắn đầu: ");
        int yearStart = Validator.ValidYearSearch(scanner, LocalDate.MIN.getYear(), LocalDate.now().getYear());
        System.out.println("Nhập vào năm kết thúc: ");
        int yearEnd = Validator.ValidYearSearch(scanner, yearStart, LocalDate.now().getYear());
        double total = invoiceService.totalYear(yearStart, yearEnd);
        if(total>0){
